...
use crate::generated::contracts::{
    ...
    IpcRequestB0, IpcWelcomeA0, OperationOutcomeA0, OperationOutcomeB0,
};
use crate::ipc::{self, Attachment, Frame, FrameKind};
use crate::operation_validation::{
    decode_and_validate_request, operation_declaration, result_uses_b0, validate_operation_response,
};
use crate::process::{
...
#[cfg(test)]
use transport::shutdown_ack_matches;
use transport::{frame_failure, reader_task, stderr_task};
...
pub enum OperationOutcome {
    A0(OperationOutcomeA0),
    B0(OperationOutcomeB0),
}

impl OperationOutcome {
    pub(crate) fn into_a0(self) -> Result<OperationOutcomeA0, GeometerClientError> {
        match self {
            Self::A0(value) => Ok(value),
            Self::B0(_) => Err(GeometerClientError::Protocol(
                "received a B0 outcome for an A0 operation".to_owned(),
            )),
        }
    }

    pub(crate) fn into_b0(self) -> Result<OperationOutcomeB0, GeometerClientError> {
        match self {
            Self::B0(value) => Ok(value),
            Self::A0(_) => Err(GeometerClientError::Protocol(
                "received an A0 outcome for a B0 operation".to_owned(),
            )),
        }
    }
}
...
fn decode_operation_response(
    ...
    let outcome = decode_outcome(welcome, operation, &frame.json)?;
...
impl GeometerClient {
    ...
    pub async fn start_execute(
        ...
        let json = match request {
            crate::operation_validation::OperationRequest::A0(request) => {
                contracts::encode_ipc_request_a0_json(&IpcRequestA0 {
                    operation: operation.to_owned(),
                    request,
                })?
            }
            crate::operation_validation::OperationRequest::B0(request) => {
                contracts::encode_ipc_request_b0_json(&IpcRequestB0 {
                    operation: operation.to_owned(),
                    request,
                })?
            }
...
fn decode_outcome(
    ...
    operation: &str,
    json: &[u8],
) -> Result<OperationOutcome, contracts::ContractError> {
    let declaration =
        operation_declaration(&welcome.operation_catalog, operation).map_err(|error| {
            contracts::ContractError::Validation {
                path: "/operation".to_owned(),
                message: error.to_string(),
            }
        })?;
    if result_uses_b0(declaration) {
        Ok(OperationOutcome::B0(
            contracts::decode_operation_outcome_b0_json(json)?,
        ))
    } else {
        Ok(OperationOutcome::A0(
            contracts::decode_operation_outcome_a0_json(json)?,
        ))
    }
}

fn outcome_operation(outcome: &OperationOutcome) -> String {
    match outcome {
        OperationOutcome::A0(OperationOutcomeA0::Success(value)) => value.operation.clone(),
        OperationOutcome::A0(OperationOutcomeA0::Failure(value)) => value.operation.clone(),
        OperationOutcome::B0(OperationOutcomeB0::Success(value)) => value.operation.clone(),
        OperationOutcome::B0(OperationOutcomeB0::Failure(value)) => value.operation.clone(),
    }
}
