...
export function decodeContractJson(data, type, declarations) {
    let text;
    try {
...
export function encodeContractJson(value, type, declarations) {
    return JSON.stringify(canonicalize(value, type, declarations, "", {}));
}
function canonicalize(value, type, declarations, path, constraints) {
    if (type.kind === "reference") {
        const declaration = declarations[type.target];
        if (!declaration)
            fail("geometer.contract.unknown_reference", path, `Unknown ${type.target}.`);
...
function canonicalizeDeclaration(value, declaration, declarations, path, constraints) {
    if (declaration.kind === "array") {
        return canonicalizeArray(value, declaration.element, declarations, path, mergeConstraints(declaration.constraints, constraints));
    }
    ...
    if (declaration.kind === "union") {
        ...
        fail("geometer.contract.union_mismatch", best?.path ?? path, best?.message ?? "Value does not match a union variant.");
    }
    if (!isObject(value))
        fail("geometer.contract.type_mismatch", path, "Expected object.");
    ...
    for (const [name, property] of Object.entries(declaration.properties)) {
        if (!Object.hasOwn(value, name)) {
            if (!property.optional) {
                fail("geometer.contract.missing_field", childPath(path, name), "Required field is missing.");
            }
            continue;
        }
        result[name] = canonicalize(value[name], property.type, declarations, childPath(path, name), property.constraints);
    }
    return result;
}
function canonicalizeArray(value, element, declarations, path, constraints) {
    if (!Array.isArray(value))
        fail("geometer.contract.type_mismatch", path, "Expected array.");
    ...
    return Array.from({ length: value.length }, (_, index) => canonicalize(value[index], element, declarations, childPath(path, String(index)), {}));
}
function validateString(value, path, constraints) {
...
function mergeConstraints(declaration, useSite) {
...
class StrictJsonParser {
    ...
    parse() {
        ...
        if (this.offset !== this.text.length)
            this.parseFailure("Trailing JSON data");
        return value;
    }
    parseValue() {
    ...
    parseObject() {
        ...
        while (true) {
            if (this.text[this.offset] !== '"')
                this.parseFailure("Expected an object key");
            const key = this.parseString();
            if (keys.has(key)) {
                throw new ContractCodecError("geometer.contract.duplicate_field", childPath("", key), "Object contains a duplicate field.");
            }
            keys.add(key);
            this.skipWhitespace();
            if (!this.consume(":"))
                this.parseFailure("Expected ':' after object key");
            this.skipWhitespace();
            ...
            if (this.consume("}"))
                return value;
            if (!this.consume(","))
                this.parseFailure("Expected ',' or '}' in object");
            this.skipWhitespace();
        }
    }
    parseArray() {
        ...
        while (true) {
            ...
            if (this.consume("]"))
                return value;
            if (!this.consume(","))
                this.parseFailure("Expected ',' or ']' in array");
            this.skipWhitespace();
        }
    }
    parseString() {
        ...
        while (this.offset < this.text.length) {
            ...
            if (!escaped && code < 0x20)
                this.parseFailure("Unescaped control character in string");
            if (!escaped && character === "\\")
                escaped = true;
            else
                escaped = false;
    ...
    parseNumber() {
        ...
        if (!match)
            return this.parseFailure("Invalid JSON number");
        this.offset += match[0].length;
        const value = Number(match[0]);
        if (!Number.isFinite(value))
            return this.parseFailure("Non-finite JSON number");
        return value;
    }
    parseLiteral(source, value) {
        if (!this.text.startsWith(source, this.offset))
            this.parseFailure(`Invalid literal ${source}`);
    ...
    parseFailure(message) {
        throw new ContractCodecError("geometer.contract.invalid_json", "", `${message} at character offset ${this.offset}.`);
    }
}
