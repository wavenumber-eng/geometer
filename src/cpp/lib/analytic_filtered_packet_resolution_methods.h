    bool publish_resolution_diagnostics()
    {
        ...
        for (const AnalyticCapsuleCoalescence& recovery : geometry_.capsule_coalescences)
        {
            ...
            if (recovery.stage_id == 0 || recovery.representative_feature_id == 0 ||
                ...
                operand->operation != 1 || operand->geometry_kind != 4 ||
                representative == nullptr || representative->operation != 1 ||
                representative->geometry_kind != 4 ||
            ...
            for (std::uint32_t stage_offset = 0;
                 stage_offset < records_.jobs[job_index_].stage_count; ++stage_offset)
            {
                const auto& stage =
            ...
            records_out_.diagnostics.push_back({65'548, 2, 15, job_id, recovery.stage_id,
                                                recovery.operand_id,
                                                recovery.representative_feature_id, 8});
        ...
        return std::adjacent_find(
                   ...
                   [](const AnalyticDiagnosticRecord& left, const AnalyticDiagnosticRecord& right)
                   {
                       return std::tie(left.job_id, left.severity, left.code, left.presence_flags,
                                       ...
                                       left.path_token) ==
                              std::tie(right.job_id, right.severity, right.code,
                                       right.presence_flags, right.stage_id, right.operand_id,
                                       right.geometry_source_id, right.path_token);
