...
pub const NORMALIZED_CATALOG_SHA256: &str =
    "8b356cc06d4df779de363d1290e1c5bcc848434b8337922b128c0555483727ed";
...
pub struct ClippingLimits {
    ...
    pub max_output_triangles: Option<u32>,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    )]
    pub max_generated_vertices: Option<u32>,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    )]
    pub max_intersections: Option<u32>,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    )]
    pub max_edge_plane_tests: Option<u32>,
}

impl Validate for ClippingLimits {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "max_output_triangles");
        if let Some(value) = &self.max_output_triangles {
            if *value < 1 {
                return Err(invalid(&field_path, "number is below its minimum"));
            }
            if *value > 2000000 {
        ...
        let field_path = child_path(path, "max_generated_vertices");
        if let Some(value) = &self.max_generated_vertices {
            if *value < 1 {
                return Err(invalid(&field_path, "number is below its minimum"));
            }
            if *value > 6000000 {
        ...
        let field_path = child_path(path, "max_intersections");
        if let Some(value) = &self.max_intersections {
            if *value < 1 {
                return Err(invalid(&field_path, "number is below its minimum"));
            }
            if *value > 8000000 {
        ...
        let field_path = child_path(path, "max_edge_plane_tests");
        if let Some(value) = &self.max_edge_plane_tests {
            if *value < 1 {
                return Err(invalid(&field_path, "number is below its minimum"));
            }
            if *value > 32000000 {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct NormalizedHalfSpacePlane {
    pub normal: IllustrationVector3,
    pub distance_mm: f64,
    pub tolerance_mm: f64,
}

impl Validate for NormalizedHalfSpacePlane {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "normal");
        let value = &self.normal;
        value.validate_at(&field_path)?;
        let field_path = child_path(path, "distance_mm");
        let value = &self.distance_mm;
        if !value.is_finite() {
            return Err(invalid(&field_path, "number must be finite"));
        }
        let field_path = child_path(path, "tolerance_mm");
        let value = &self.tolerance_mm;
        if !value.is_finite() {
            return Err(invalid(&field_path, "number must be finite"));
        }
        if *value < 0_f64 {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct NormalizedClipping {
    pub planes: Vec<NormalizedHalfSpacePlane>,
    pub cap_policy: String,
    pub max_output_triangles: u32,
    pub max_generated_vertices: u32,
    pub max_intersections: u32,
    pub max_edge_plane_tests: u32,
}

impl Validate for NormalizedClipping {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "planes");
        let value = &self.planes;
        if value.is_empty() {
            return Err(invalid(&field_path, "array is shorter than its minimum"));
        }
        if value.len() > 16 {
        ...
        let field_path = child_path(path, "cap_policy");
        let value = &self.cap_policy;
        if value != "none" {
            return Err(invalid(
                &field_path,
                "literal value does not match the contract",
...
pub struct FragmentMetadata {
    ...
    pub clipping: Option<NormalizedClipping>,
    pub input_triangles: u32,
    pub output_triangles: u32,
    pub fragment_sha256: String,
    pub linework_geometry_sha256: String,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    )]
    pub raw_attachment_sha256: Option<String>,
}

impl Validate for FragmentMetadata {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "clipping");
        if let Some(value) = &self.clipping {
            value.validate_at(&field_path)?;
        }
        let field_path = child_path(path, "input_triangles");
        let value = &self.input_triangles;
        if *value > 2000000 {
            return Err(invalid(&field_path, "number exceeds its maximum"));
        }
        let field_path = child_path(path, "output_triangles");
        let value = &self.output_triangles;
        if *value > 2000000 {
            return Err(invalid(&field_path, "number exceeds its maximum"));
        }
        let field_path = child_path(path, "fragment_sha256");
        let value = &self.fragment_sha256;
        if value.len() < 64 {
            return Err(invalid(&field_path, "string is shorter than its minimum"));
        }
        if value.len() > 64 {
        ...
        let field_path = child_path(path, "linework_geometry_sha256");
        let value = &self.linework_geometry_sha256;
        if value.len() < 64 {
            return Err(invalid(&field_path, "string is shorter than its minimum"));
        }
        if value.len() > 64 {
        ...
        let field_path = child_path(path, "raw_attachment_sha256");
        if let Some(value) = &self.raw_attachment_sha256 {
            if value.len() < 64 {
                return Err(invalid(&field_path, "string is shorter than its minimum"));
            }
            if value.len() > 64 {
...
impl Validate for HalfSpacePlane {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "normal");
        let value = &self.normal;
        value.validate_at(&field_path)?;
        ...
        let field_path = child_path(path, "tolerance_mm");
        if let Some(value) = &self.tolerance_mm {
            if !value.is_finite() {
                return Err(invalid(&field_path, "number must be finite"));
            }
            if *value < 0_f64 {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct IllustrationClipping {
    pub planes: Vec<HalfSpacePlane>,
    pub cap_policy: String,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
    ...
    pub limits: Option<ClippingLimits>,
}

impl Validate for IllustrationClipping {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "planes");
        let value = &self.planes;
        if value.is_empty() {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ModelTessellationOptionsA0 {
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
...
impl Validate for ModelTessellationOptionsA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ModelAttachmentIllustrationSourceA0 {
    pub kind: String,
    pub attachment: String,
    #[serde(
        default,
    ...
    pub transform: Option<IllustrationMatrix4x4>,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    )]
    pub material_override: Option<MeshIllustrationMaterial>,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    )]
    pub tessellation: Option<ModelTessellationOptionsA0>,
}

impl Validate for ModelAttachmentIllustrationSourceA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "kind");
        let value = &self.kind;
        if value != "model" {
        ...
        let field_path = child_path(path, "material_override");
        if let Some(value) = &self.material_override {
            value.validate_at(&field_path)?;
        }
        let field_path = child_path(path, "tessellation");
        if let Some(value) = &self.tessellation {
            value.validate_at(&field_path)?;
        }
        Ok(())
    }
...
pub type IllustrationVector2Mm = [f64; 2];

#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct IllustrationProfileLineA0 {
    pub kind: String,
}

impl Validate for IllustrationProfileLineA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "kind");
        let value = &self.kind;
        if value != "line" {
...
impl Validate for PlanarArcSweep {
    fn validate_at(&self, _path: &str) -> Result<(), ContractError> {
        Ok(())
    }
}
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct IllustrationProfileCircularArcA0 {
    pub kind: String,
    pub center_mm: IllustrationVector2Mm,
    pub sweep: PlanarArcSweep,
}

impl Validate for IllustrationProfileCircularArcA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "kind");
        let value = &self.kind;
        if value != "circular_arc" {
        ...
        let field_path = child_path(path, "center_mm");
        let value = &self.center_mm;
        value.validate_at(&field_path)?;
        let field_path = child_path(path, "sweep");
        let value = &self.sweep;
        value.validate_at(&field_path)?;
        Ok(())
    }
}
...
#[serde(untagged)]
pub enum IllustrationProfileSegmentA0 {
    Line(IllustrationProfileLineA0),
    CircularArc(IllustrationProfileCircularArcA0),
}

impl<'de> Deserialize<'de> for IllustrationProfileSegmentA0 {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        let raw = Box::<serde_json::value::RawValue>::deserialize(deserializer)?;
        if let Ok(value) = serde_json::from_str::<IllustrationProfileLineA0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::Line(value));
            }
        }
        if let Ok(value) = serde_json::from_str::<IllustrationProfileCircularArcA0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::CircularArc(value));
            }
        }
        Err(serde::de::Error::custom(
            "value does not match any IllustrationProfileSegmentA0 variant",
        ))
    }
}

impl Validate for IllustrationProfileSegmentA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        match self {
            Self::Line(value) => value.validate_at(path),
            Self::CircularArc(value) => value.validate_at(path),
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct IllustrationProfileRingA0 {
    pub points_mm: Vec<IllustrationVector2Mm>,
    pub segments: Vec<IllustrationProfileSegmentA0>,
}

impl Validate for IllustrationProfileRingA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "points_mm");
        let value = &self.points_mm;
        if value.len() < 3 {
            return Err(invalid(&field_path, "array is shorter than its minimum"));
        }
        if value.len() > 131072 {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct IllustrationProfileRegionA0 {
    pub outer: IllustrationProfileRingA0,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    )]
    pub holes: Option<Vec<IllustrationProfileRingA0>>,
}

impl Validate for IllustrationProfileRegionA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "outer");
        let value = &self.outer;
        value.validate_at(&field_path)?;
        let field_path = child_path(path, "holes");
        if let Some(value) = &self.holes {
            if value.len() > 131071 {
                return Err(invalid(&field_path, "array exceeds its maximum"));
            }
            for (index, item) in value.iter().enumerate() {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct AnalyticExtrusionA0 {
    pub kind: String,
    pub id: String,
    pub regions: Vec<IllustrationProfileRegionA0>,
    pub z_min_mm: f64,
    pub z_max_mm: f64,
    pub material: MeshIllustrationMaterial,
}

impl Validate for AnalyticExtrusionA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "kind");
        let value = &self.kind;
        if value != "extrusion" {
        ...
        let field_path = child_path(path, "regions");
        let value = &self.regions;
        if value.is_empty() {
            return Err(invalid(&field_path, "array is shorter than its minimum"));
        }
        if value.len() > 65536 {
        ...
        let field_path = child_path(path, "z_min_mm");
        let value = &self.z_min_mm;
        if !value.is_finite() {
            return Err(invalid(&field_path, "number must be finite"));
        }
        let field_path = child_path(path, "z_max_mm");
        let value = &self.z_max_mm;
        if !value.is_finite() {
            return Err(invalid(&field_path, "number must be finite"));
        }
        let field_path = child_path(path, "material");
        let value = &self.material;
        value.validate_at(&field_path)?;
        Ok(())
    }
}
...
impl Validate for AnalyticCylinderA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "kind");
        let value = &self.kind;
        if value != "cylinder" {
        ...
        let value = &self.center_mm;
        value.validate_at(&field_path)?;
        let field_path = child_path(path, "radius_mm");
        let value = &self.radius_mm;
        if !value.is_finite() {
            return Err(invalid(&field_path, "number must be finite"));
        }
        if *value <= 0_f64 {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct AnalyticSphereA0 {
    pub kind: String,
    pub id: String,
    pub center_mm: IllustrationVector3,
    pub radius_mm: f64,
    pub material: MeshIllustrationMaterial,
}

impl Validate for AnalyticSphereA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "kind");
        let value = &self.kind;
        if value != "sphere" {
...
#[derive(Clone, Debug, PartialEq, Serialize)]
#[serde(untagged)]
pub enum AnalyticPrimitiveA0 {
    Extrusion(AnalyticExtrusionA0),
    Cylinder(AnalyticCylinderA0),
    Sphere(AnalyticSphereA0),
}

impl<'de> Deserialize<'de> for AnalyticPrimitiveA0 {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        let raw = Box::<serde_json::value::RawValue>::deserialize(deserializer)?;
        if let Ok(value) = serde_json::from_str::<AnalyticExtrusionA0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::Extrusion(value));
            }
        }
        if let Ok(value) = serde_json::from_str::<AnalyticCylinderA0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::Cylinder(value));
            }
        }
        if let Ok(value) = serde_json::from_str::<AnalyticSphereA0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::Sphere(value));
            }
        }
        Err(serde::de::Error::custom(
            "value does not match any AnalyticPrimitiveA0 variant",
        ))
    }
}

impl Validate for AnalyticPrimitiveA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        match self {
            Self::Extrusion(value) => value.validate_at(path),
            Self::Cylinder(value) => value.validate_at(path),
            Self::Sphere(value) => value.validate_at(path),
        }
    }
}
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct AnalyticDefinitionA0 {
    pub id: String,
    pub primitives: Vec<AnalyticPrimitiveA0>,
}

impl Validate for AnalyticDefinitionA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "id");
        let value = &self.id;
        if value.is_empty() {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct AnalyticOccurrenceA0 {
    pub id: String,
...
impl Validate for AnalyticOccurrenceA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "id");
        let value = &self.id;
        if value.is_empty() {
        ...
        let field_path = child_path(path, "definition_id");
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct AnalyticSceneA0 {
    pub definitions: Vec<AnalyticDefinitionA0>,
    pub occurrences: Vec<AnalyticOccurrenceA0>,
}

impl Validate for AnalyticSceneA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct AnalyticIllustrationLimitsA0 {
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    )]
    pub max_reached_definitions: Option<u32>,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    )]
    pub max_reached_occurrences: Option<u32>,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    )]
    pub max_reached_primitives: Option<u32>,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    )]
    pub max_reached_rings: Option<u32>,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    )]
    pub max_reached_points: Option<u32>,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    )]
    pub max_generated_curve_samples: Option<u32>,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    )]
    pub max_definition_triangles: Option<u32>,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    )]
    pub max_topology_candidate_pairs: Option<u32>,
}

impl Validate for AnalyticIllustrationLimitsA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "max_reached_definitions");
        if let Some(value) = &self.max_reached_definitions {
            if *value < 1 {
                return Err(invalid(&field_path, "number is below its minimum"));
            }
            if *value > 65536 {
        ...
        let field_path = child_path(path, "max_reached_occurrences");
        if let Some(value) = &self.max_reached_occurrences {
            if *value < 1 {
                return Err(invalid(&field_path, "number is below its minimum"));
            }
            if *value > 65536 {
        ...
        let field_path = child_path(path, "max_reached_primitives");
        if let Some(value) = &self.max_reached_primitives {
            if *value < 1 {
                return Err(invalid(&field_path, "number is below its minimum"));
            }
            if *value > 65536 {
        ...
        let field_path = child_path(path, "max_reached_rings");
        if let Some(value) = &self.max_reached_rings {
            if *value < 1 {
                return Err(invalid(&field_path, "number is below its minimum"));
            }
            if *value > 262144 {
        ...
        let field_path = child_path(path, "max_reached_points");
        if let Some(value) = &self.max_reached_points {
            if *value < 1 {
                return Err(invalid(&field_path, "number is below its minimum"));
            }
            if *value > 2000000 {
        ...
        let field_path = child_path(path, "max_generated_curve_samples");
        if let Some(value) = &self.max_generated_curve_samples {
            if *value < 1 {
                return Err(invalid(&field_path, "number is below its minimum"));
            }
            if *value > 2000000 {
        ...
        let field_path = child_path(path, "max_definition_triangles");
        if let Some(value) = &self.max_definition_triangles {
            if *value < 1 {
                return Err(invalid(&field_path, "number is below its minimum"));
            }
            if *value > 2000000 {
        ...
        let field_path = child_path(path, "max_topology_candidate_pairs");
        if let Some(value) = &self.max_topology_candidate_pairs {
            if *value < 1 {
                return Err(invalid(&field_path, "number is below its minimum"));
            }
            if *value > 100000000 {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct AnalyticLoweringOptionsA0 {
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    ...
    pub limits: Option<AnalyticIllustrationLimitsA0>,
}

impl Validate for AnalyticLoweringOptionsA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "linear_deflection_mm");
        if let Some(value) = &self.linear_deflection_mm {
            if !value.is_finite() {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct AnalyticIllustrationSourceA0 {
    pub kind: String,
    pub scene: AnalyticSceneA0,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    )]
    pub lowering: Option<AnalyticLoweringOptionsA0>,
}

impl Validate for AnalyticIllustrationSourceA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "kind");
        let value = &self.kind;
        if value != "analytic" {
        ...
        let field_path = child_path(path, "scene");
        let value = &self.scene;
        value.validate_at(&field_path)?;
        let field_path = child_path(path, "lowering");
        if let Some(value) = &self.lowering {
            value.validate_at(&field_path)?;
        }
        Ok(())
    }
...
#[derive(Clone, Debug, PartialEq, Serialize)]
#[serde(untagged)]
pub enum ModelIllustrationSourceA0 {
    ModelSource(ModelAttachmentIllustrationSourceA0),
    AnalyticSource(AnalyticIllustrationSourceA0),
}

impl<'de> Deserialize<'de> for ModelIllustrationSourceA0 {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        let raw = Box::<serde_json::value::RawValue>::deserialize(deserializer)?;
        if let Ok(value) = serde_json::from_str::<ModelAttachmentIllustrationSourceA0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::ModelSource(value));
            }
        }
        if let Ok(value) = serde_json::from_str::<AnalyticIllustrationSourceA0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::AnalyticSource(value));
            }
        }
        Err(serde::de::Error::custom(
            "value does not match any ModelIllustrationSourceA0 variant",
        ))
    }
}

impl Validate for ModelIllustrationSourceA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        match self {
            Self::ModelSource(value) => value.validate_at(path),
            Self::AnalyticSource(value) => value.validate_at(path),
        }
    }
}
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ModelIllustrationLineworkOptionsA0 {
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    ...
    pub outline_width_mm: Option<f64>,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    )]
    pub detail_width_mm: Option<f64>,
}

impl Validate for ModelIllustrationLineworkOptionsA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "fast");
        if let Some(value) = &self.fast {
            value.validate_at(&field_path)?;
        }
        let field_path = child_path(path, "outline_width_mm");
        if let Some(value) = &self.outline_width_mm {
            if !value.is_finite() {
                return Err(invalid(&field_path, "number must be finite"));
            }
            if *value <= 0_f64 {
        ...
        let field_path = child_path(path, "detail_width_mm");
        if let Some(value) = &self.detail_width_mm {
            if !value.is_finite() {
                return Err(invalid(&field_path, "number must be finite"));
            }
            if *value <= 0_f64 {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ModelIllustrationWorkLimitsA0 {
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    )]
    pub max_visibility_candidate_pairs: Option<u32>,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    )]
    pub max_drawing_commands: Option<u32>,
}

impl Validate for ModelIllustrationWorkLimitsA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "max_visibility_candidate_pairs");
        if let Some(value) = &self.max_visibility_candidate_pairs {
            if *value < 1 {
                return Err(invalid(&field_path, "number is below its minimum"));
            }
            if *value > 100000000 {
        ...
        let field_path = child_path(path, "max_drawing_commands");
        if let Some(value) = &self.max_drawing_commands {
            if *value < 1 {
                return Err(invalid(&field_path, "number is below its minimum"));
            }
            if *value > 2000000 {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ModelIllustrationGeometryRequestA0 {
    pub schema: String,
    pub source: ModelIllustrationSourceA0,
    ...
    pub linework: Option<ModelIllustrationLineworkOptionsA0>,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
        skip_serializing_if = "Option::is_none"
    ...
    pub work_limits: Option<ModelIllustrationWorkLimitsA0>,
}

impl Validate for ModelIllustrationGeometryRequestA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "schema");
        let value = &self.schema;
        if value != "geometry.model_illustration_geometry.request.a0" {
            return Err(invalid(
                &field_path,
                "literal value does not match the contract",
            ));
        ...
        let field_path = child_path(path, "linework");
        if let Some(value) = &self.linework {
            value.validate_at(&field_path)?;
        }
        ...
        let field_path = child_path(path, "work_limits");
        if let Some(value) = &self.work_limits {
            value.validate_at(&field_path)?;
        }
        Ok(())
    }
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ModelIllustrationRequestA0 {
    pub schema: String,
    pub source: ModelIllustrationSourceA0,
    pub view: MeshIllustrationView,
    #[serde(
...
impl Validate for ModelIllustrationRequestA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "schema");
        let value = &self.schema;
        if value != "geometry.model_illustration.request.a0" {
            return Err(invalid(
                &field_path,
                "literal value does not match the contract",
            ));
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct MeshIllustrationRequestA0 {
    pub schema: String,
    pub view: MeshIllustrationView,
    #[serde(
        default,
...
impl Validate for MeshIllustrationRequestA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "schema");
        let value = &self.schema;
        if value != "geometry.mesh_illustration.request.a0" {
            return Err(invalid(
                &field_path,
                "literal value does not match the contract",
            ));
...
pub enum IpcRequestValueA0 {
    ModelIllustrationGeometry(ModelIllustrationGeometryRequestA0),
    ModelIllustration(ModelIllustrationRequestA0),
...
impl<'de> Deserialize<'de> for IpcRequestValueA0 {
    ...
    {
        let raw = Box::<serde_json::value::RawValue>::deserialize(deserializer)?;
        if let Ok(value) = serde_json::from_str::<ModelIllustrationGeometryRequestA0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::ModelIllustrationGeometry(value));
            }
        }
        if let Ok(value) = serde_json::from_str::<ModelIllustrationRequestA0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::ModelIllustration(value));
            }
        }
...
impl Validate for IpcRequestValueA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        match self {
            Self::ModelIllustrationGeometry(value) => value.validate_at(path),
            Self::ModelIllustration(value) => value.validate_at(path),
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ModelIllustrationGeometryRequestB0 {
    pub schema: String,
    pub source: ModelIllustrationSourceA0,
    pub view: MeshIllustrationView,
    #[serde(
...
impl Validate for ModelIllustrationGeometryRequestB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "schema");
        let value = &self.schema;
        if value != "geometry.model_illustration_geometry.request.b0" {
            return Err(invalid(
                &field_path,
                "literal value does not match the contract",
            ));
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ModelIllustrationRequestB0 {
    pub schema: String,
    pub source: ModelIllustrationSourceA0,
    pub view: MeshIllustrationView,
    #[serde(
...
impl Validate for ModelIllustrationRequestB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "schema");
        let value = &self.schema;
        if value != "geometry.model_illustration.request.b0" {
            return Err(invalid(
                &field_path,
                "literal value does not match the contract",
            ));
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct MeshIllustrationGeometryRequestB0 {
    pub schema: String,
    pub view: MeshIllustrationView,
    #[serde(
        default,
...
impl Validate for MeshIllustrationGeometryRequestB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "schema");
        let value = &self.schema;
        if value != "geometry.mesh_illustration_geometry.request.b0" {
            return Err(invalid(
                &field_path,
                "literal value does not match the contract",
            ));
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct MeshIllustrationRequestB0 {
    pub schema: String,
    pub view: MeshIllustrationView,
    #[serde(
        default,
...
impl Validate for MeshIllustrationRequestB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "schema");
        let value = &self.schema;
        if value != "geometry.mesh_illustration.request.b0" {
            return Err(invalid(
                &field_path,
                "literal value does not match the contract",
            ));
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct MeshHlrProjectionRequestB0 {
    pub schema: String,
    #[serde(
        default,
        deserialize_with = "deserialize_optional_non_null",
...
impl Validate for MeshHlrProjectionRequestB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "schema");
        let value = &self.schema;
        if value != "geometry.mesh_hlr_projection.request.b0" {
            return Err(invalid(
                &field_path,
                "literal value does not match the contract",
            ));
...
#[derive(Clone, Debug, PartialEq, Serialize)]
#[serde(untagged)]
pub enum IpcRequestValueB0 {
    ModelIllustrationGeometry(ModelIllustrationGeometryRequestB0),
    ModelIllustration(ModelIllustrationRequestB0),
    MeshIllustrationGeometry(MeshIllustrationGeometryRequestB0),
    MeshIllustration(MeshIllustrationRequestB0),
    MeshHlrProjection(MeshHlrProjectionRequestB0),
}

impl<'de> Deserialize<'de> for IpcRequestValueB0 {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        let raw = Box::<serde_json::value::RawValue>::deserialize(deserializer)?;
        if let Ok(value) = serde_json::from_str::<ModelIllustrationGeometryRequestB0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::ModelIllustrationGeometry(value));
            }
        }
        if let Ok(value) = serde_json::from_str::<ModelIllustrationRequestB0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::ModelIllustration(value));
            }
        }
        if let Ok(value) = serde_json::from_str::<MeshIllustrationGeometryRequestB0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::MeshIllustrationGeometry(value));
            }
        }
        if let Ok(value) = serde_json::from_str::<MeshIllustrationRequestB0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::MeshIllustration(value));
            }
        }
        if let Ok(value) = serde_json::from_str::<MeshHlrProjectionRequestB0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::MeshHlrProjection(value));
            }
        }
        Err(serde::de::Error::custom(
            "value does not match any IpcRequestValueB0 variant",
        ))
    }
}

impl Validate for IpcRequestValueB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        match self {
            Self::ModelIllustrationGeometry(value) => value.validate_at(path),
            Self::ModelIllustration(value) => value.validate_at(path),
            Self::MeshIllustrationGeometry(value) => value.validate_at(path),
            Self::MeshIllustration(value) => value.validate_at(path),
            Self::MeshHlrProjection(value) => value.validate_at(path),
        }
    }
}
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct IpcRequestB0 {
    pub operation: String,
    pub request: IpcRequestValueB0,
}

impl Validate for IpcRequestB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "operation");
        let value = &self.operation;
        if value.is_empty() {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct MeshCollectionHlrSource {
    pub kind: String,
    pub hash: String,
}

impl Validate for MeshCollectionHlrSource {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "kind");
        let value = &self.kind;
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct HlrProjectionResultB0 {
    pub schema: String,
    pub units: String,
    pub empty: bool,
    pub source: MeshCollectionHlrSource,
    pub views: Vec<HlrProjectedView>,
    pub timings: HlrProjectionTimings,
    pub fragment: FragmentMetadata,
}

impl Validate for HlrProjectionResultB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "schema");
        let value = &self.schema;
        if value != "geometry.hlr_projection.result.b0" {
            return Err(invalid(
                &field_path,
                "literal value does not match the contract",
            ));
        ...
        let value = &self.timings;
        value.validate_at(&field_path)?;
        let field_path = child_path(path, "fragment");
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct MeshIllustrationInputB0 {
    pub schema: String,
    pub meshes: Vec<MeshIllustrationMesh>,
    pub view: MeshIllustrationView,
    #[serde(
...
impl Validate for MeshIllustrationInputB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "schema");
        let value = &self.schema;
        if value != "geometry.mesh_illustration.input.b0" {
            return Err(invalid(
                &field_path,
                "literal value does not match the contract",
            ));
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct MeshIllustrationResultB0 {
    pub schema: String,
    pub empty: bool,
    pub svg: String,
    pub stats: MeshIllustrationRenderStats,
...
impl Validate for MeshIllustrationResultB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "schema");
        let value = &self.schema;
        if value != "geometry.mesh_illustration.result.b0" {
            return Err(invalid(
                &field_path,
                "literal value does not match the contract",
            ));
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct IllustrationGeometryAttachmentB0 {
    pub attachment: String,
    pub schema: String,
    pub byte_length: u32,
    pub sha256: String,
...
impl Validate for IllustrationGeometryAttachmentB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "attachment");
        let value = &self.attachment;
        if value != "illustration_geometry" {
        ...
        let field_path = child_path(path, "schema");
        let value = &self.schema;
        if value != "geometry.mesh_illustration.geometry.b0" {
            return Err(invalid(
                &field_path,
                "literal value does not match the contract",
            ));
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct MeshIllustrationGeometryB0 {
    pub schema: String,
    pub length_unit: String,
    pub empty: bool,
    pub view: MeshIllustrationView,
    ...
    pub bounds: Option<IllustrationGeometryBounds>,
    pub surfaces: Vec<IllustrationGeometrySurface>,
    pub lines: Vec<IllustrationGeometryLine>,
    pub presentation: IllustrationGeometryPresentation,
    pub stats: MeshIllustrationRenderStats,
...
impl Validate for MeshIllustrationGeometryB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "schema");
        let value = &self.schema;
        if value != "geometry.mesh_illustration.geometry.b0" {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct MeshIllustrationGeometryInputB0 {
    pub schema: String,
    pub length_unit: String,
    pub meshes: Vec<MeshIllustrationMesh>,
    pub view: MeshIllustrationView,
...
impl Validate for MeshIllustrationGeometryInputB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "schema");
        let value = &self.schema;
        if value != "geometry.mesh_illustration_geometry.input.b0" {
            return Err(invalid(
                &field_path,
                "literal value does not match the contract",
            ));
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct MeshIllustrationGeometryResultB0 {
    pub schema: String,
    pub empty: bool,
    pub geometry: IllustrationGeometryAttachmentB0,
    pub stats: MeshIllustrationRenderStats,
    pub fragment: FragmentMetadata,
    pub warnings: Vec<String>,
}

impl Validate for MeshIllustrationGeometryResultB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "schema");
        let value = &self.schema;
        if value != "geometry.mesh_illustration_geometry.result.b0" {
            return Err(invalid(
                &field_path,
                "literal value does not match the contract",
            ));
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct AnalyticSourceSummaryA0 {
    pub kind: String,
    pub definitions: u32,
    pub occurrences: u32,
    pub primitives: u32,
...
impl Validate for AnalyticSourceSummaryA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "kind");
        let value = &self.kind;
        if value != "analytic" {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
pub enum ModelAttachmentMediaTypeA0 {
    #[serde(rename = "application/step")]
    ApplicationStep,
    #[serde(rename = "model/step")]
    ModelStep,
}

impl Validate for ModelAttachmentMediaTypeA0 {
    fn validate_at(&self, _path: &str) -> Result<(), ContractError> {
        Ok(())
    }
}
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ModelAttachmentSourceSummaryA0 {
    pub kind: String,
    pub media_type: ModelAttachmentMediaTypeA0,
    pub source_sha256: String,
    pub meshes: u32,
    pub triangles: u32,
}

impl Validate for ModelAttachmentSourceSummaryA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "kind");
        let value = &self.kind;
        if value != "model" {
...
pub type ModelIllustrationBounds3MmA0 = [f64; 6];

impl Validate for ModelIllustrationBounds3MmA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        for (index, item) in self.iter().enumerate() {
            let item_path = child_path(path, &index.to_string());
            if !item.is_finite() {
...
#[derive(Clone, Debug, PartialEq, Serialize)]
#[serde(untagged)]
pub enum ModelIllustrationSourceSummaryA0 {
    ModelSource(ModelAttachmentSourceSummaryA0),
    AnalyticSource(AnalyticSourceSummaryA0),
}

impl<'de> Deserialize<'de> for ModelIllustrationSourceSummaryA0 {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        let raw = Box::<serde_json::value::RawValue>::deserialize(deserializer)?;
        if let Ok(value) = serde_json::from_str::<ModelAttachmentSourceSummaryA0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::ModelSource(value));
            }
        }
        if let Ok(value) = serde_json::from_str::<AnalyticSourceSummaryA0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::AnalyticSource(value));
            }
        }
        Err(serde::de::Error::custom(
            "value does not match any ModelIllustrationSourceSummaryA0 variant",
        ))
    }
}

impl Validate for ModelIllustrationSourceSummaryA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        match self {
            Self::ModelSource(value) => value.validate_at(path),
            Self::AnalyticSource(value) => value.validate_at(path),
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ModelIllustrationTimingsA0 {
    pub source_preparation_ms: f64,
    pub linework_ms: f64,
    pub illustration_ms: f64,
    pub attachment_encoding_ms: f64,
}

impl Validate for ModelIllustrationTimingsA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "source_preparation_ms");
        let value = &self.source_preparation_ms;
        if !value.is_finite() {
            return Err(invalid(&field_path, "number must be finite"));
        }
        if *value < 0_f64 {
        ...
        let field_path = child_path(path, "linework_ms");
        let value = &self.linework_ms;
        if !value.is_finite() {
            return Err(invalid(&field_path, "number must be finite"));
        }
        if *value < 0_f64 {
        ...
        let field_path = child_path(path, "illustration_ms");
        let value = &self.illustration_ms;
        if !value.is_finite() {
            return Err(invalid(&field_path, "number must be finite"));
        }
        if *value < 0_f64 {
        ...
        let field_path = child_path(path, "attachment_encoding_ms");
        let value = &self.attachment_encoding_ms;
        if !value.is_finite() {
            return Err(invalid(&field_path, "number must be finite"));
        }
        if *value < 0_f64 {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ModelIllustrationGeometryResultA0 {
    pub schema: String,
    pub geometry: IllustrationGeometryAttachment,
    pub bounds_mm: ModelIllustrationBounds3MmA0,
    pub source: ModelIllustrationSourceSummaryA0,
    pub stats: MeshIllustrationRenderStats,
    pub timings: ModelIllustrationTimingsA0,
    pub warnings: Vec<String>,
}

impl Validate for ModelIllustrationGeometryResultA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "schema");
        let value = &self.schema;
        if value != "geometry.model_illustration_geometry.result.a0" {
            return Err(invalid(
                &field_path,
                "literal value does not match the contract",
            ));
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ModelIllustrationResultA0 {
    pub schema: String,
    pub svg: String,
    pub bounds_mm: ModelIllustrationBounds3MmA0,
    pub source: ModelIllustrationSourceSummaryA0,
...
impl Validate for ModelIllustrationResultA0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "schema");
        let value = &self.schema;
        if value != "geometry.model_illustration.result.a0" {
            return Err(invalid(
                &field_path,
                "literal value does not match the contract",
            ));
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ModelIllustrationGeometryResultB0 {
    pub schema: String,
    pub empty: bool,
    pub geometry: IllustrationGeometryAttachmentB0,
    #[serde(
    ...
    pub bounds_mm: Option<ModelIllustrationBounds3MmA0>,
    pub source: ModelIllustrationSourceSummaryA0,
    pub stats: MeshIllustrationRenderStats,
    pub timings: ModelIllustrationTimingsA0,
    pub fragment: FragmentMetadata,
...
impl Validate for ModelIllustrationGeometryResultB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "schema");
        let value = &self.schema;
        if value != "geometry.model_illustration_geometry.result.b0" {
            return Err(invalid(
                &field_path,
                "literal value does not match the contract",
            ));
        ...
        value.validate_at(&field_path)?;
        let field_path = child_path(path, "bounds_mm");
        if let Some(value) = &self.bounds_mm {
            value.validate_at(&field_path)?;
        }
        let field_path = child_path(path, "source");
        let value = &self.source;
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ModelIllustrationResultB0 {
    pub schema: String,
    pub empty: bool,
    pub svg: String,
    #[serde(
...
impl Validate for ModelIllustrationResultB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "schema");
        let value = &self.schema;
        if value != "geometry.model_illustration.result.b0" {
            return Err(invalid(
                &field_path,
                "literal value does not match the contract",
            ));
...
pub enum OperationResultValueA0 {
    ModelIllustrationGeometry(ModelIllustrationGeometryResultA0),
    ModelIllustration(ModelIllustrationResultA0),
...
impl<'de> Deserialize<'de> for OperationResultValueA0 {
    ...
    {
        let raw = Box::<serde_json::value::RawValue>::deserialize(deserializer)?;
        if let Ok(value) = serde_json::from_str::<ModelIllustrationGeometryResultA0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::ModelIllustrationGeometry(value));
            }
        }
        if let Ok(value) = serde_json::from_str::<ModelIllustrationResultA0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::ModelIllustration(value));
            }
        }
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct OperationFailureB0 {
    pub operation: String,
    pub ok: bool,
    pub diagnostics: Vec<DiagnosticA0>,
}

impl Validate for OperationFailureB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "operation");
        let value = &self.operation;
        if value.is_empty() {
...
#[derive(Clone, Debug, PartialEq, Serialize)]
#[serde(untagged)]
pub enum OperationResultValueB0 {
    ModelIllustrationGeometry(ModelIllustrationGeometryResultB0),
    ModelIllustration(ModelIllustrationResultB0),
    MeshIllustrationGeometry(MeshIllustrationGeometryResultB0),
    MeshIllustration(MeshIllustrationResultB0),
    MeshHlrProjection(HlrProjectionResultB0),
}

impl<'de> Deserialize<'de> for OperationResultValueB0 {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        let raw = Box::<serde_json::value::RawValue>::deserialize(deserializer)?;
        if let Ok(value) = serde_json::from_str::<ModelIllustrationGeometryResultB0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::ModelIllustrationGeometry(value));
            }
        }
        if let Ok(value) = serde_json::from_str::<ModelIllustrationResultB0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::ModelIllustration(value));
            }
        }
        if let Ok(value) = serde_json::from_str::<MeshIllustrationGeometryResultB0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::MeshIllustrationGeometry(value));
            }
        }
        if let Ok(value) = serde_json::from_str::<MeshIllustrationResultB0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::MeshIllustration(value));
            }
        }
        if let Ok(value) = serde_json::from_str::<HlrProjectionResultB0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::MeshHlrProjection(value));
            }
        }
        Err(serde::de::Error::custom(
            "value does not match any OperationResultValueB0 variant",
        ))
    }
}

impl Validate for OperationResultValueB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        match self {
            Self::ModelIllustrationGeometry(value) => value.validate_at(path),
            Self::ModelIllustration(value) => value.validate_at(path),
...
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct OperationSuccessB0 {
    pub operation: String,
    pub ok: bool,
    pub result: OperationResultValueB0,
}

impl Validate for OperationSuccessB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        let field_path = child_path(path, "operation");
        let value = &self.operation;
        if value.is_empty() {
...
#[derive(Clone, Debug, PartialEq, Serialize)]
#[serde(untagged)]
pub enum OperationOutcomeB0 {
    Success(OperationSuccessB0),
    Failure(OperationFailureB0),
}

impl<'de> Deserialize<'de> for OperationOutcomeB0 {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        let raw = Box::<serde_json::value::RawValue>::deserialize(deserializer)?;
        if let Ok(value) = serde_json::from_str::<OperationSuccessB0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::Success(value));
            }
        }
        if let Ok(value) = serde_json::from_str::<OperationFailureB0>(raw.get()) {
            if value.validate_at("").is_ok() {
                return Ok(Self::Failure(value));
            }
        }
        Err(serde::de::Error::custom(
            "value does not match any OperationOutcomeB0 variant",
        ))
    }
}

impl Validate for OperationOutcomeB0 {
    fn validate_at(&self, path: &str) -> Result<(), ContractError> {
        match self {
            Self::Success(value) => value.validate_at(path),
            Self::Failure(value) => value.validate_at(path),
...
pub fn decode_ipc_request_b0_json(data: &[u8]) -> Result<IpcRequestB0, ContractError> {
    decode_json(data)
}

pub fn encode_ipc_request_b0_json(value: &IpcRequestB0) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}

pub fn decode_hlr_projection_result_b0_json(
    data: &[u8],
) -> Result<HlrProjectionResultB0, ContractError> {
    decode_json(data)
}

pub fn encode_hlr_projection_result_b0_json(
    value: &HlrProjectionResultB0,
) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}

pub fn decode_mesh_hlr_projection_request_b0_json(
    data: &[u8],
) -> Result<MeshHlrProjectionRequestB0, ContractError> {
    decode_json(data)
}

pub fn encode_mesh_hlr_projection_request_b0_json(
    value: &MeshHlrProjectionRequestB0,
) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}
...
pub fn decode_mesh_illustration_input_b0_json(
    data: &[u8],
) -> Result<MeshIllustrationInputB0, ContractError> {
    decode_json(data)
}

pub fn encode_mesh_illustration_input_b0_json(
    value: &MeshIllustrationInputB0,
) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}

pub fn decode_mesh_illustration_request_b0_json(
    data: &[u8],
) -> Result<MeshIllustrationRequestB0, ContractError> {
    decode_json(data)
}

pub fn encode_mesh_illustration_request_b0_json(
    value: &MeshIllustrationRequestB0,
) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}

pub fn decode_mesh_illustration_result_b0_json(
    data: &[u8],
) -> Result<MeshIllustrationResultB0, ContractError> {
    decode_json(data)
}

pub fn encode_mesh_illustration_result_b0_json(
    value: &MeshIllustrationResultB0,
) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}
...
pub fn decode_mesh_illustration_geometry_b0_json(
    data: &[u8],
) -> Result<MeshIllustrationGeometryB0, ContractError> {
    decode_json(data)
}

pub fn encode_mesh_illustration_geometry_b0_json(
    value: &MeshIllustrationGeometryB0,
) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}

pub fn decode_mesh_illustration_geometry_input_b0_json(
    data: &[u8],
) -> Result<MeshIllustrationGeometryInputB0, ContractError> {
    decode_json(data)
}

pub fn encode_mesh_illustration_geometry_input_b0_json(
    value: &MeshIllustrationGeometryInputB0,
) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}

pub fn decode_mesh_illustration_geometry_request_b0_json(
    data: &[u8],
) -> Result<MeshIllustrationGeometryRequestB0, ContractError> {
    decode_json(data)
}

pub fn encode_mesh_illustration_geometry_request_b0_json(
    value: &MeshIllustrationGeometryRequestB0,
) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}

pub fn decode_mesh_illustration_geometry_result_b0_json(
    data: &[u8],
) -> Result<MeshIllustrationGeometryResultB0, ContractError> {
    decode_json(data)
}

pub fn encode_mesh_illustration_geometry_result_b0_json(
    value: &MeshIllustrationGeometryResultB0,
) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}

pub fn decode_mesh_illustration_request_a0_json(
    data: &[u8],
) -> Result<MeshIllustrationRequestA0, ContractError> {
    decode_json(data)
}

pub fn encode_mesh_illustration_request_a0_json(
    value: &MeshIllustrationRequestA0,
) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}
...
pub fn decode_model_illustration_geometry_request_a0_json(
    data: &[u8],
) -> Result<ModelIllustrationGeometryRequestA0, ContractError> {
    decode_json(data)
}

pub fn encode_model_illustration_geometry_request_a0_json(
    value: &ModelIllustrationGeometryRequestA0,
) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}

pub fn decode_model_illustration_geometry_result_a0_json(
    data: &[u8],
) -> Result<ModelIllustrationGeometryResultA0, ContractError> {
    decode_json(data)
}

pub fn encode_model_illustration_geometry_result_a0_json(
    value: &ModelIllustrationGeometryResultA0,
) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}

pub fn decode_model_illustration_request_a0_json(
    data: &[u8],
) -> Result<ModelIllustrationRequestA0, ContractError> {
    decode_json(data)
}

pub fn encode_model_illustration_request_a0_json(
    value: &ModelIllustrationRequestA0,
) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}

pub fn decode_model_illustration_result_a0_json(
    data: &[u8],
) -> Result<ModelIllustrationResultA0, ContractError> {
    decode_json(data)
}

pub fn encode_model_illustration_result_a0_json(
    value: &ModelIllustrationResultA0,
) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}

pub fn decode_model_illustration_geometry_request_b0_json(
    data: &[u8],
) -> Result<ModelIllustrationGeometryRequestB0, ContractError> {
    decode_json(data)
}

pub fn encode_model_illustration_geometry_request_b0_json(
    value: &ModelIllustrationGeometryRequestB0,
) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}

pub fn decode_model_illustration_geometry_result_b0_json(
    data: &[u8],
) -> Result<ModelIllustrationGeometryResultB0, ContractError> {
    decode_json(data)
}

pub fn encode_model_illustration_geometry_result_b0_json(
    value: &ModelIllustrationGeometryResultB0,
) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}

pub fn decode_model_illustration_request_b0_json(
    data: &[u8],
) -> Result<ModelIllustrationRequestB0, ContractError> {
    decode_json(data)
}

pub fn encode_model_illustration_request_b0_json(
    value: &ModelIllustrationRequestB0,
) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}

pub fn decode_model_illustration_result_b0_json(
    data: &[u8],
) -> Result<ModelIllustrationResultB0, ContractError> {
    decode_json(data)
}

pub fn encode_model_illustration_result_b0_json(
    value: &ModelIllustrationResultB0,
) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}
...
pub fn decode_operation_outcome_b0_json(data: &[u8]) -> Result<OperationOutcomeB0, ContractError> {
    decode_json(data)
}

pub fn encode_operation_outcome_b0_json(
    value: &OperationOutcomeB0,
) -> Result<Vec<u8>, ContractError> {
    encode_json(value)
}
