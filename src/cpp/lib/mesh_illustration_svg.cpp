...
struct Svg
{
    ...
    {
        const auto color = safe_illustration_color(layer.fill),
                   opacity = number_text(layer.opacity);
    ...
    {
        const auto color = safe_illustration_color(line.color);
                       ...
                       const contracts::MeshIllustrationSvgOptions& options)
{
    ...
    if (!style.transparent_background)
        svg.append("<rect width=\"" + svg_width + "\" height=\"" + svg_height + "\" fill=\"" +
                   escape_xml(safe_illustration_color(style.background)) + "\"/>");
