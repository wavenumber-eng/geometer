...
export function validateIpcOutcomeOperationPair(outcome) {
    if (!outcome.ok)
        return;
    ...
    const contract = validatePayloadProjection(outcome.result, declaration.runtimeDispatch, projection);
    if (contract !== declaration.resultContract) {
        throw new GeometerIpcProtocolError(`IPC result payload ${contract} does not match operation ${outcome.operation}.`);
    }
}
export function encodeGeometerIpcFrame(frame) {
    ...
    for (const attachment of frame.attachments) {
        ...
        attachmentBytes = checkedAdd(attachmentBytes, 16 + name.byteLength + mediaType.byteLength + attachment.data.byteLength, "IPC attachment section");
...
export class GeometerIpcFrameDecoder {
    ...
    pushOne(chunk) {
        ...
        if (this.pending.byteLength < HEADER_BYTES)
            return undefined;
        const header = decodeFixedHeader(this.pending.peek(HEADER_BYTES), this.limits);
        if (this.pending.byteLength < header.frameBytes)
            return undefined;
    ...
    setLimits(limits) {
        ...
        if (maxima.some(([value, maximum]) => !Number.isInteger(value) || value <= 0 || value > maximum)) {
...
function decodeFixedHeader(bytes, limits) {
    for (let index = 0; index < MAGIC.byteLength; index += 1) {
        if (bytes[index] !== MAGIC[index])
            throw new GeometerIpcProtocolError("Invalid IPC magic.");
    }
    const view = new DataView(bytes.buffer, bytes.byteOffset, HEADER_BYTES);
    if (view.getUint16(8, true) !== HEADER_BYTES ||
...
function decodeQueuedFrame(queue, header, limits) {
    ...
    for (let index = 0; index < header.attachmentCount; index += 1) {
        ...
        const view = new DataView(attachmentHeader.buffer, attachmentHeader.byteOffset, attachmentHeader.byteLength);
        ...
        if (names.has(name))
            throw new GeometerIpcProtocolError(`Duplicate IPC attachment ${name}.`);
        names.add(name);
...
function validateKindAndRequestId(kind, requestId) {
    if (!Number.isInteger(kind) || kind < 1 || kind > 10) {
        throw new GeometerIpcProtocolError("Unknown IPC frame kind.");
    }
    if (requestId < 0n || requestId > 0xffffffffffffffffn) {
...
function operationDeclaration(operation) {
    ...
    return operationCatalog[operation];
}
function validatePayloadProjection(value, runtimeDispatch, expectedProjection, fallback) {
    const hasPacket = "packet" in value;
    if (runtimeDispatch === "logical_dto") {
        if (hasPacket) {
            throw new GeometerIpcProtocolError("A logical DTO operation cannot carry a packed attachment projection.");
        }
    }
    else {
        ...
        if (typeof packet !== "object" ||
            ...
            throw new GeometerIpcProtocolError("Packed attachment projection metadata differs from the operation declaration.");
        }
    }
