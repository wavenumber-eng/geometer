...
// Internal overlay builder and its bounded sweep phases.
class OverlayBuilder
{
  ...
  private:
    ...
    bool validate_inputs()
    {
        ...
        for (std::size_t index = 0; index < narrow_.intersections.size(); ++index)
        {
            ...
            for (std::uint8_t point_index = 0; point_index < value.point_count; ++point_index)
            {
                ...
                const std::uint64_t expected_integer_intersection =
                    policy_ == analytic_execution_detail::TopologyPolicy::
                                    resolution_50nm_preserve_integer_construction_endpoints &&
                            first_curve.kind == AnalyticAtomicCurveKind::line &&
                            second_curve.kind == AnalyticAtomicCurveKind::line &&
                            singleton_integer_point(value.points[point_index])
                        ? analytic_integer_line_intersection_token(
                              first_curve.construction_carrier_id,
                              second_curve.construction_carrier_id)
                        : 0;
                std::uint64_t expected_symmetric_root = 0;
                if (value.point_count == 2 &&
                    ((first_curve.kind == AnalyticAtomicCurveKind::line &&
                      ...
                      second_curve.kind == AnalyticAtomicCurveKind::line)))
                {
                    const AnalyticAtomicCurveNm& line =
                        first_curve.kind == AnalyticAtomicCurveKind::line ? first_curve
                                                                          : second_curve;
                    const AnalyticAtomicCurveNm& arc =
                        first_curve.kind == AnalyticAtomicCurveKind::circular_arc ? first_curve
                                                                                  : second_curve;
                    const std::uint8_t other_index = point_index == 0 ? 1 : 0;
                    const auto& current_x = value.points[point_index].x;
                    const auto& other_x = value.points[other_index].x;
                    const bool current_is_left = current_x.upper < other_x.lower;
                    const bool current_is_right = other_x.upper < current_x.lower;
                    if (line.construction_horizontal_mirror_id != 0 &&
                        arc.has_integer_certificate &&
                        arc.integer_center.y == line.construction_horizontal_mirror_axis_y &&
                        (current_is_left || current_is_right))
                        expected_symmetric_root =
                            analytic_symmetric_line_circle_root_x_column_token(
                                line.construction_horizontal_mirror_id, arc.construction_carrier_id,
                                current_is_right);
                }
                if (token != 0 && token != expected_pair_column &&
                    token != expected_first_vertical && token != expected_second_vertical &&
                    token != expected_symmetric_root && token != expected_integer_intersection)
                    return fail(AnalyticFilteredOverlayError::invalid_argument);
            }
    ...
    bool build_groups()
    {
        ...
        curve_group_.resize(geometry_.curves.size());
        for (std::uint32_t group_index = 0; group_index < groups_.size(); ++group_index)
        {
            const CarrierGroup& group = groups_[group_index];
            for (std::uint32_t local = 0; local < group.curve_count; ++local)
            {
                curve_local_[curve_order_[group.curve_begin + local]] = local;
                curve_group_[curve_order_[group.curve_begin + local]] = group_index;
            }
        }
        return true;
    }
                                        ...
                                        const AnalyticFilteredPointNm& event) const noexcept
    {
        if (curve.kind == AnalyticAtomicCurveKind::circular_arc)
        {
            if (curve.has_endpoint_authoritative_x_monotone_certificate &&
                curve.curve_index <= curve_group_.size() &&
                groups_[curve_group_[curve.curve_index - 1]].curve_count == 1)
            {
                const double x = midpoint(event.x);
                return {curve.endpoint_authoritative_upper_branch ? -x : x, 0.0};
            }
        ...
        for (CarrierGroup& group : groups_)
        {
            ...
            for (std::uint32_t local = 0; local < group.event_count; ++local)
            {
                ...
                if (merge)
                {
                    ...
                    else if (unique_events_.back().has_endpoint)
                        representative = unique_events_.back().point;
                    else if (event.role == EventRole::domain_start ||
                             event.role == EventRole::domain_end)
                        representative = event.point;
    ...
    std::vector<std::uint32_t> curve_group_;
