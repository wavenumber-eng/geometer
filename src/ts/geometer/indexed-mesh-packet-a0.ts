export const INDEXED_TRIANGLE_MESH_MEDIA_TYPE =
  "application/vnd.wavenumber.geometer.indexed-triangle-mesh" as const;
export const INDEXED_TRIANGLE_MESH_PACKET_FORMAT =
  "geometry.indexed_triangle_mesh.packet.a0" as const;
...
export function encodeIndexedTriangleMeshA0Packet(mesh: IndexedTriangleMeshA0): Uint8Array {
  ...
  if (packetBytes > MAX_PACKET_BYTES) fail("indexed-mesh packet exceeds 268 MiB.");
  ...
  for (let index = 0; index < mesh.indices.length; index += 1) {
    ...
    if (vertex >= vertexCount) fail("triangle index is outside the vertex table.");
...
export function decodeIndexedTriangleMeshA0Packet(
  packet: Uint8Array,
): DecodedIndexedTriangleMeshA0 {
  ...
  for (let index = 0; index < MAGIC.length; index += 1) {
    if (packet[index] !== MAGIC[index]) fail("indexed-mesh packet magic is invalid.");
  }
  ...
  for (let index = 0; index < positions.length; index += 1) {
    ...
    if (!Number.isFinite(coordinate)) fail("indexed-mesh position is non-finite.");
  ...
  let sourceFaces: Uint32Array | undefined;
  if (hasSourceFaces) {
...
function assertUint32(value: number | undefined, label: string): asserts value is number {
  if (value === undefined || !Number.isInteger(value) || value < 0 || value > 0xffff_ffff) {
    fail(`${label} must be an unsigned 32-bit integer.`);
  }
}

function exactOffset(value: bigint): number {
  if (value > BigInt(Number.MAX_SAFE_INTEGER)) fail("packet offset exceeds JavaScript range.");
  return Number(value);
}

function assertZeroRange(packet: Uint8Array, begin: number, end: number): void {
  if (begin < 0 || end < begin || end > packet.byteLength) fail("packet range is invalid.");
  for (let index = begin; index < end; index += 1) {
    if (packet[index] !== 0) fail("packet alignment or reserved bytes must be zero.");
  }
}
