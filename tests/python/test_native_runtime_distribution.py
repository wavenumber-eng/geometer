...
def test_release_workflow_uses_filtered_native_runtime_packager() -> None:
    ...
    assert '"package_release_artifacts.py"' in candidate
    assert '"native"' in candidate
    assert "make_archive('out/native-" not in workflow
    assert '"wasm"' in candidate
    assert "out/release-candidate/wasm/wasm-dist.zip" in workflow
