...
VECTOR_PREFIX = "ANALYTIC_FILTERED_BATCH_VECTOR="
...
def _vector(command: list[str]) -> bytes:
    ...
    raise RuntimeError("filtered-batch test omitted its parity vector")
...
def main() -> int:
    ...
    if len(native_vector) != expected["batch_vector_bytes"]:
        raise RuntimeError("filtered-batch vector byte length differs from manifest")
    if digest != expected["batch_vector_sha256"]:
        raise RuntimeError("filtered-batch SHA-256 differs from manifest")
    print(f"analytic filtered batch parity: {len(native_vector)} bytes sha256={digest}")
    return 0
