...
// Internal result-packet builder and publication phases.
class PacketBuilder
{
  public:
  ...
  private:
                 ...
                 SourceExpectation expectation, bool require_nonempty)
    {
        ...
        if (range.begin > sources.size() || range.count > sources.size() - range.begin ||
            ...
            return false;
        if (logical_source_reference_expansions_ >
                kAnalyticMaximumLogicalSourceReferenceExpansions ||
            range.count > kAnalyticMaximumLogicalSourceReferenceExpansions -
                              logical_source_reference_expansions_)
            return fail_resource();
        logical_source_reference_expansions_ += range.count;
    ...
    std::uint64_t logical_source_reference_expansions_ = 0;
