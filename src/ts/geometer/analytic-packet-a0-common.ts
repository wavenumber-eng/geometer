...
export const MAX_LENGTH_NM = 1_000_000_000_000n;
...
export const REQUEST_TABLES = [24, 32, 24, 32, 4, 32, 24, 40, 32, 40, 48, 32, 24] as const;
export const RESULT_TABLES = [48, 56, 32, 48, 32, 4, 24, 8, 8, 32, 48, 32, 32, 4] as const;

export const diagnosticCodes: Readonly<Record<number, JobDiagnosticCode>> = {
...
export const pathTokens: readonly (JobDiagnosticPath | undefined)[] = [
  undefined,
  "request_jobs",
  "job_id",
  "job_stages",
...
export const sourceRoles: Readonly<Record<number, SourceRole>> = {
  0: "none",
  1: "authored_line",
...
export const outcomeKinds: Readonly<Record<number, OperandOutcomeKind>> = {
...
export function record(size: number, write: (view: DataView) => void): Uint8Array {
  const value = new Uint8Array(size);
  write(new DataView(value.buffer));
  return value;
}
export function recordView(table: DirectoryTable, index: number): DataView {
...
export function putU32(view: DataView, offset: number, value: number, label: string): void {
  if (!Number.isSafeInteger(value) || value < 0 || value > U32_NONE)
    fail(`${label} is outside uint32.`);
  view.setUint32(offset, value, true);
}
export function putU64(
  view: DataView,
  offset: number,
  value: bigint,
  label: string,
...
export function putI64(view: DataView, offset: number, value: bigint, label: string): void {
  if (typeof value !== "bigint" || value < I64_MIN || value > I64_MAX)
    fail(`${label} must be an int64 bigint.`);
  view.setBigInt64(offset, value, true);
}
export function unsigned(value: bigint, label: string, nonzero: boolean): void {
  if (typeof value !== "bigint" || value < (nonzero ? 1n : 0n) || value > U64_MAX)
    fail(`${label} must be a${nonzero ? " nonzero" : ""} uint64 bigint.`);
}
export function length(value: bigint, label: string): void {
...
export function compareSource(left: SourceRecord, right: SourceRecord): number {
...
export function compareNumberArrays(left: readonly number[], right: readonly number[]): number {
...
export function strictlyIncreasing(values: readonly bigint[], label: string): void {
  for (let index = 1; index < values.length; index += 1)
    if (required(values[index - 1], label) >= required(values[index], label))
      fail(`${label} are not strictly increasing.`);
}
export function oneBased(values: readonly bigint[], label: string): void {
...
export function flags(size: number): boolean[] {
  return Array.from({ length: size }, () => false);
}
export function sparseMap(indexes: readonly number[]): Map<number, number> {
...
export function mapped(mapping: ReadonlyMap<number, number>, index: number): number {
  const value = mapping.get(index);
  if (value === undefined) fail("Standalone closure contains an unmapped reference.");
  return value;
}
export function remapHandle(handle: number, mapping: ReadonlyMap<number, number>): number {
