...
import type { Vec2 } from "./demo-tooling/index.js";
...
export interface PcbVia extends BoardPoint {
  readonly id: number;
}
...
export interface PcbDemoState {
  readonly board: readonly BoardPoint[];
  readonly vias: readonly PcbVia[];
  readonly traces: readonly PcbTrace[];
  readonly nextViaId: number;
  readonly nextTraceId: number;
}

export type PcbLayerKind = "base" | "clearance" | "thermal";

export function classifyPcbLayerJob(jobId: bigint): PcbLayerKind {
  if (jobId === 31n) return "base";
  if (jobId >= 40_000n && jobId < 50_000n) return "clearance";
  if (jobId >= 50_000n) return "thermal";
  throw new RangeError(`Unknown PCB layer job ${jobId}.`);
}

export function initialPcbDemoState(): PcbDemoState {
  return {
    board: [
      { x: 0, y: 0 },
      { x: 26, y: 0 },
...
export function moveVia(state: PcbDemoState, id: number, point: BoardPoint): PcbDemoState {
  return {
    ...state,
...
export function addVia(state: PcbDemoState, point: BoardPoint): PcbDemoState {
  return {
    ...state,
...
export function moveBoardVertex(
  state: PcbDemoState,
  index: number,
  point: BoardPoint,
): PcbDemoState {
...
export function addTrace(state: PcbDemoState, points: readonly BoardPoint[]): PcbDemoState {
...
export function snapPointTo45(anchor: BoardPoint, candidate: BoardPoint): BoardPoint {
...
export function makePcbPolygonPourRequest(
  state: PcbDemoState,
): AnalyticPlanarBooleanBatchRequestA0 {
  let nextOperand = 10_000n;
  const operandId = (): bigint => {
    nextOperand += 1n;
    return nextOperand;
  };
  const pads: AnalyticPlanarOperand[] = footprintPads().map((pad) =>
    disk(operandId(), pad, 520_000n),
  );
  const clearances: AnalyticPlanarOperand[] = [];
  ...
  const thermalJobs = state.vias.flatMap((via) => {
    ...
    const thermalCopper: AnalyticPlanarOperand[] = [
...
export function footprintPads(): readonly BoardPoint[] {
  return [
    { x: 4.5, y: 9.5 },
    { x: 4.5, y: 7.9 },
  ];
...
function region(operandId: bigint, points: readonly BoardPoint[]): PlanarRegionOperand {
  const base = operandId * 100n;
  const vertices: readonly AuthoredVertex[] = points.map((point, index) => ({
...
function disk(operandId: bigint, center: BoardPoint, radius: bigint): AnalyticPlanarOperand {
  return {
    operand_id: operandId,
    kind: "disk",
...
function stripRegion(
  ...
  if (length === 0) return region(operandId, [
...
function singleOperandJob(jobId: bigint, operand: AnalyticPlanarOperand) {
  return {
    job_id: jobId,
    stages: [{ stage_id: jobId * 10n, operation: "union" as const, operands: [operand] }],
  };
}

function pointNm(point: BoardPoint): { readonly x: bigint; readonly y: bigint } {
  return {
...
function gridNm(point: BoardPoint): BoardPoint {
  return {
