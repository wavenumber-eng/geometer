...
export interface FastHlrBuildStats {
  buildMs: number;
  meshes: number;
  triangles: number;
...
export interface FastHlrFrameStats extends FastHlrBuildStats {
  cpuMs: number;
  gpuMs: number | null;
  frameMs: number;
  fps: number;
...
const DEFAULT_STYLE: FastHlrStyle = {
...
/**
 * Retained, raster-only hidden-line scene. Faces populate the ordinary GPU
 * depth buffer; candidate edges render afterwards with depth testing enabled.
 * The polygon offset is the prototype's explicit self-occlusion bias.
 */
export class FastHlrModel {
  readonly root: THREE.Object3D;
  readonly stats: FastHlrBuildStats;
  ...
  constructor(source: THREE.Object3D, requested: Partial<FastHlrStyle> = {}) {
    ...
    for (const mesh of meshes) {
      ...
      // deliberately a prototype policy, not a production topology contract.
  ...
  setStyle(requested: Partial<FastHlrStyle>): void {
...
export class FastHlrViewport {
  readonly renderer: THREE.WebGLRenderer;

  private readonly scene = new THREE.Scene();
  private model: FastHlrModel | null = null;
  private style: FastHlrStyle = { ...DEFAULT_STYLE };
  ...
  setSource(source: THREE.Object3D | null): FastHlrBuildStats | null {
    this.model?.dispose();
    this.model = null;
    this.samples = 0;
    this.meanCpuMs = 0;
    ...
    this.model = new FastHlrModel(source, this.style);
  ...
  render(camera: THREE.Camera): FastHlrFrameStats | null {
  ...
  dispose(): void {
    ...
    this.model = null;
    if (this.gl)
      for (const query of this.pendingTimerQueries) this.gl.deleteQuery(query);
