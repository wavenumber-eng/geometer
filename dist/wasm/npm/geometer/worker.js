...
export class GeometerWorkerClient {
    ...
    async analyticPlanarBooleanBatch(request) {
        ...
        const input = declaration.inputAttachments.find((item) => item.name === requestProjection.attachment_name);
        const output = declaration.outputAttachments.find((item) => item.name === resultProjection.attachment_name);
        ...
        const response = await this.execute(declaration.identity, JSON.stringify({
...
function validateWorkerOperationResponse(operation, response) {
    if (response.outcome.operation !== operation) {
        throw new GeometerWorkerError(`Expected ${operation} response, received ${response.outcome.operation}.`);
    }
    if (!response.outcome.ok) {
        if (response.attachments.length !== 0) {
    ...
    const declarations = operationCatalog;
    ...
    for (const attachment of response.attachments) {
        ...
        if (!expected.media_types.includes(attachment.mediaType)) {
            throw new GeometerWorkerError(`Worker returned incompatible media type ${attachment.mediaType} for ${attachment.name}.`);
        }
    ...
    for (const expected of declaration.outputAttachments) {
        if (expected.required && !seen.has(expected.name)) {
            throw new GeometerWorkerError(`Worker response is missing required attachment ${expected.name}.`);
        }
    }
