...
const runtime = requireElement<HTMLElement>("runtime-label");
const slotSlider = requireElement<HTMLInputElement>("slot-position");
const slotValue = requireElement<HTMLElement>("slot-value");
const resetButton = requireElement<HTMLButtonElement>("reset-demo");
const context = requireCanvasContext(canvas);

let activeClient: GeometerWorkerClient | undefined;
let latestResult: SuccessfulJobResult | undefined;
let running = false;
...
function scheduleSolve(positionMm: number): void {
  if (running && pendingPosition !== undefined) replacedSolves += 1;
  ...
  try {
    ...
    while (pendingPosition !== undefined) {
      ...
      const result = await client.analyticPlanarBooleanBatch(
        makeAnalyticPolygonPourRequest(position),
      );
      ...
      if (job === undefined || job.status !== "success") {
        throw new Error(
          job?.diagnostics.map((item) => item.code).join(", ") || "Solver returned no job.",
        );
      }
      if (
    ...
    running = false;
    if (pendingPosition !== undefined) void drainLatest();
  }
}

function render(job: SuccessfulJobResult): void {
  ...
  const fragments = new Map(
    job.directed_fragments.map((fragment) => [fragment.fragment_id, fragment]),
  );
  context.beginPath();
...
function appendRing(
  ...
  for (const fragmentId of fragmentIds) {
    ...
    if (fragment !== undefined) appendFragment(fragment, vertices, false);
  }
  context.closePath();
}

function appendFragment(
  fragment: DirectedFragment,
  vertices: ReadonlyMap<bigint, ResultVertex>,
  move: boolean,
): void {
  ...
  if (move) context.moveTo(sx, sy);
  if (fragment.kind === "line") context.lineTo(ex, ey);
  else appendArc(fragment, sx, sy, ex, ey);
}

function appendArc(
  fragment: ResultCircularArcFragment,
  sx: number,
  sy: number,
  ex: number,
  ey: number,
): void {
  const arc = resolveReflectedCanvasArc(
    sx,
    sy,
    ex,
    ey,
    Number(fragment.radius_nm),
    fragment.direction,
    fragment.major_arc,
  );
  if (arc === undefined) {
...
function updateMetrics(
  ...
  failureProbe: import("@wavenumber/geometer").AnalyticPlanarBooleanJobResult | undefined,
  elapsed: number,
): void {
  ...
  updateCounters();
  const subtractionEdges = job.directed_fragments.filter(
    (fragment) => fragment.surviving_subtraction_sources.sources.length > 0,
  ).length;
  ...
  for (const fragment of job.directed_fragments)
    for (const source of [
      ...
      sourceKeys.add(
        `${source.kind}/${source.role}/${source.operand_id}/${source.primary_id}/${source.secondary_id}`,
      );
  setText("source-incidence", `${sourceKeys.size} canonical boundary source identities`);
  const lineage = requireElement<HTMLUListElement>("lineage-events");
  lineage.replaceChildren(
    ...job.operand_outcomes.slice(0, 6).map((event) => {
...
function resize(): void {
  if (latestResult !== undefined) render(latestResult);
}
