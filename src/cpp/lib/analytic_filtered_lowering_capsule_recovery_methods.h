        ...
        for (std::uint32_t offset = 0; offset < stage.operand_count; ++offset)
        {
                    ...
                    for (std::int64_t c = -1; c <= 1; ++c)
                        for (std::int64_t d = -1; d <= 1; ++d)
                        {
                            if (!charge_work(ordered_index_work(buckets.size())))
                                return false;
                            const CapsuleBucketKey key{capsule.width_nm, base[0] + a, base[1] + b,
                                                       base[2] + c, base[3] + d};
                            ...
                            for (const CapsuleRepresentative& candidate : found->second)
                            {
                                ...
                                if (endpoint_adjustment(canonical, candidate.canonical, squared) &&
                                    (selected == nullptr ||
            ...
            if (selected == nullptr)
            {
                if (!charge_work(ordered_index_work(buckets.size()) + 1))
                    return false;
                const CapsuleBucketKey key{capsule.width_nm, base[0], base[1], base[2], base[3]};
                buckets[key].push_back({operand_index, operand.operand_id, operand.geometry_index,
                                        capsule.feature_id, canonical});
                continue;
            }
