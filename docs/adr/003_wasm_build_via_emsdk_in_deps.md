...
4. Copies `geometer.js` and `geometer.wasm` to `dist/`.
...
- `ENVIRONMENT=node` — CLI target, not browser (browser module is a future addition).
- `EXIT_RUNTIME=1` — clean exit after main() returns.
