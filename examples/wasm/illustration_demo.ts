...
interface IllustrationWorkerResponse {
  id: number;
  ok: boolean;
  operation?: "step-to-glb" | "mesh-shadow";
  glbBuffer?: ArrayBuffer;
  projection?: HlrProjection;
  error?: string;
  timings?: { glbMs?: number; hlrMs?: number };
}
...
const els = {
  ...
  meshQuality: required<HTMLSelectElement>("illustrationMeshQuality"),
...
const state: {
  ...
  pendingWorker: Map<
    number,
    {
      resolve: (value: IllustrationWorkerResponse) => void;
      reject: (reason: Error) => void;
    }
  >;
  uploadedUrls: string[];
  projectionCache: Map<string, HlrProjection>;
  validation: boolean;
...
function setBusy(message: string | null, presentation: "overlay" | "indicator" = "overlay"): void {
...
function currentMeshSettings(): MeshSettings {
  return {
    linearDeflectionMm: boundedNumberInput(els.linearDeflection, 0.001, 10, 0.1),
    angularDeflectionDegrees: boundedNumberInput(
...
function surfaceMeshKey(settings: MeshSettings): string {
...
function applyMeshSettings(settings: MeshSettings): void {
...
function cancelLazyHlrLinework(): void {
  state.lineworkRequest += 1;
  if (state.lineworkBusyRequest === null) return;
...
async function prepareIllustration(reason: string): Promise<void> {
  ...
  try {
    ...
    if (requestId !== state.prepareRequest || state.root !== root || state.model !== model) return;
    cancelLazyHlrLinework();
    ...
    if (requestId === state.prepareRequest) setBusy(null);
  }
}

async function updateHlrVisibility(): Promise<void> {
  ...
  setBusy(`Tracing ${model.name} with Geometer HLR...`, "indicator");
  try {
    const view: MeshIllustrationView = {
    ...
    const linework = await loadHlrLinework(model, view, currentModelTransform(root));
    if (
      requestId !== state.lineworkRequest ||
      state.scene !== scene ||
      state.model !== model ||
      state.root !== root
    )
...
function updatePreviewLighting(): void {
...
async function modelGlbBuffer(model: DemoModel): Promise<{
  buffer: ArrayBuffer;
  meshMs: number;
  source: "prebuilt" | "remeshed" | "cached" | "fixed-glb";
}> {
  const settings = currentMeshSettings();
...
function stepWorker(): Worker {
  ...
  worker.onmessage = (event: MessageEvent) => {
    const data = event.data as IllustrationWorkerResponse;
    const pending = state.pendingWorker.get(data.id);
    if (!pending) return;
    state.pendingWorker.delete(data.id);
    if (data.ok) pending.resolve(data);
    else pending.reject(new Error(data.error ?? "STEP adapter Worker failed."));
  };
...
function runStepWorker(
  message: Record<string, unknown>,
  stepBuffer: ArrayBuffer,
): Promise<IllustrationWorkerResponse> {
...
async function convertStep(
  stepBuffer: ArrayBuffer,
  settings: MeshSettings = currentMeshSettings(),
): Promise<{ buffer: ArrayBuffer; glbMs: number }> {
  ...
  if (!result.glbBuffer) throw new Error("STEP conversion Worker returned no GLB data.");
  return { buffer: result.glbBuffer, glbMs: result.timings?.glbMs ?? 0 };
}

async function modelStepBuffer(model: DemoModel): Promise<ArrayBuffer> {
  if (model.stepBuffer) return model.stepBuffer;
  if (!model.step) throw new Error(`${model.name} has no STEP source for HLR linework.`);
...
function projectionCacheKey(
  ...
  modelTransform: readonly number[],
): string {
  const settings = currentMeshSettings();
  const values = [
    ...view.direction,
...
async function loadHlrLinework(
  model: DemoModel,
  view: MeshIllustrationView,
  modelTransform: number[],
): Promise<{
  outlineSegments: Array<{ points: [[number, number], [number, number]] }>;
  detailSegments: Array<{ points: [[number, number], [number, number]] }>;
  hlrMs: number;
}> {
  ...
  if (!projection) {
    ...
    if (!result.projection) throw new Error("HLR Worker returned no projection data.");
  ...
  ): Array<{ points: [[number, number], [number, number]] }> =>
    segments
...
function markMeshQualityCustom(): void {
  els.meshQuality.value = "custom";
  applyMeshSettings(currentMeshSettings());
}

async function applyMeshQualityPreset(): Promise<void> {
  const quality = els.meshQuality.value as MeshQualityId;
...
async function rebuildHlrLinework(): Promise<void> {
  ...
  if (state.root) await prepareIllustration("HLR mesh");
}
