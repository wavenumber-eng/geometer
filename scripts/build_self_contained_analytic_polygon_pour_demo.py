#!/usr/bin/env python3
"""Build the analytic polygon-pour demo as one directly openable HTML file."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Fail if the committed standalone artifact differs from a fresh build.",
    )
...
def make_worker_source(worker_bundle: str, glue: bytes) -> str:
    return "\n".join(
        [
            ...
            "try {",
            "  importScripts(geometerScriptUrl);",
            "} finally {",
            "  URL.revokeObjectURL(geometerScriptUrl);",
            "}",
            worker_bundle,
        ]
    )
...
def build_html() -> str:
    ...
    with tempfile.TemporaryDirectory(prefix="analytic-pour-standalone-", dir=DEMOS) as raw_stage:
...
def main() -> None:
    ...
    if args.check:
        if not OUTPUT.is_file() or OUTPUT.read_bytes() != encoded:
            raise SystemExit(
                "Standalone analytic polygon-pour demo is stale; run "
                "python scripts/build_self_contained_analytic_polygon_pour_demo.py"
            )
        print(f"Standalone analytic polygon-pour demo is current ({digest}).")
        return
