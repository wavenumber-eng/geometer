...
import type {
  GeometerWorkerAttachmentMessage,
  GeometerWorkerRequestMessage,
  GeometerWorkerResponseMessage,
  GeometerWorkerSerializedError,
} from "./worker.js";
...
export function startGeometerWorkerHost(
  factory: EmscriptenGeometerFactory,
  scope: GeometerWorkerScope,
  options: GeometerWorkerHostOptions = {},
): void {
  let clientPromise: ReturnType<typeof createGeometerWasmClient> | undefined;
  ...
  scope.addEventListener("message", (event) => {
    ...
    operationQueue = operationQueue.then(
      () => dispatchSerialized(request),
      () => dispatchSerialized(request),
    );
  });

  async function initialize(
    request: Extract<GeometerWorkerRequestMessage, { kind: "initialize" }>,
  ): Promise<void> {
    if (clientPromise) {
  ...
  async function dispatchSerialized(
    request: Exclude<GeometerWorkerRequestMessage, { kind: "initialize" }>,
  ): Promise<void> {
    try {
      ...
      if (!clientPromise) throw new Error("Geometer Worker is not initialized.");
      const client = await clientPromise;
...
function isWorkerRequest(value: unknown): value is GeometerWorkerRequestMessage {
  ...
  if (value.kind === "shutdown") return true;
...
function toTransferAttachment(
  attachment: GeometerOperationAttachment,
): GeometerWorkerAttachmentMessage {
  if (
    attachment.data.buffer instanceof ArrayBuffer &&
...
function serializeError(error: unknown): GeometerWorkerSerializedError {
