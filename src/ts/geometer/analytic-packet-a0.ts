...
const MAX_LENGTH_NM = 1_000_000_000_000n;
...
const REQUEST_TABLES = [24, 32, 24, 32, 4, 32, 24, 40, 32, 40, 48, 32, 24] as const;
const RESULT_TABLES = [48, 56, 32, 48, 32, 4, 24, 8, 8, 32, 48, 32, 32, 4] as const;

const diagnosticCodes: Readonly<Record<number, JobDiagnosticCode>> = {
...
const pathTokens: readonly (JobDiagnosticPath | undefined)[] = [
  undefined,
  "request_jobs",
  "job_id",
  "job_stages",
...
const sourceRoles: Readonly<Record<number, SourceRole>> = {
  0: "none",
  1: "authored_line",
...
const outcomeKinds: Readonly<Record<number, OperandOutcomeKind>> = {
...
export function encodeAnalyticPlanarBooleanBatchRequestA0Packet(
  ...
  const tables = REQUEST_TABLES.map<MutableTable>((recordBytes, index) => ({
    kind: index + 1,
    recordBytes,
    records: [],
  }));
  const table = (kind: number): MutableTable => required(tables[kind - 1], `request table ${kind}`);
  const ids = new Map<string, Set<bigint>>();
  const uniqueId = (space: string, value: bigint): void => {
    unsigned(value, `${space} id`, true);
    const values = ids.get(space) ?? new Set<bigint>();
    if (values.has(value)) fail(`Duplicate ${space} id ${value}.`);
    values.add(value);
    ids.set(space, values);
  };

  const addRing = (ring: PlanarRing | PlanarPath, open: boolean): number => {
    const isPath = "path_id" in ring;
    if (isPath !== open) fail("Ring/path ownership does not match its geometry kind.");
    ...
    if (
      ring.segments.length > MAX_RING_SEGMENTS ||
      (open
  ...
  const addOperand = (operand: AnalyticPlanarOperand): void => {
    ...
    } else fail("Unknown analytic operand geometry kind.");
    table(3).records.push(
      record(24, (view) => {
  ...
  for (const job of jobs) {
    ...
    if (!Array.isArray(job.stages)) fail("Job stages must be an array.");
    ...
    for (const stage of job.stages) {
      ...
      if (!Array.isArray(stage.operands)) fail("Stage operands must be an array.");
      if (table(3).records.length + stage.operands.length > MAX_OPERANDS)
        fail("Analytic request exceeds its operand limit.");
      const operands = [...stage.operands].sort((left, right) =>
        compareBigint(left.operand_id, right.operand_id),
      );
      for (const operand of operands) addOperand(operand);
      table(2).records.push(
        record(32, (view) => {
  ...
  const jobIds = ids.get("job") ?? new Set<bigint>();
  for (const query of [...request.relationship_queries].sort((left, right) =>
    compareBigint(left.query_id, right.query_id),
  )) {
...
export async function decodeAnalyticPlanarBooleanBatchResultA0Packet(
  bytes: Uint8Array,
  ...
  if (!equalBytes(canonicalBytes, bytes)) fail("Result packet is not canonically encoded.");
  const digests: string[] = [];
  for (let index = 0; index < records.jobs.length; index += 1) {
    digests.push(
      await sha256Hex(
        encodeStandalone(records, index, required(selections[index], "job selection")),
      ),
    );
  }
  const jobResults = records.jobs.map((_, index) =>
    projectJob(
      records,
      index,
      required(selections[index], "job selection"),
      required(digests[index], "job digest"),
    ),
  );
  const relationshipResults = records.relationships.map((relationship) =>
    projectRelationship(records, relationship),
  );
  const header = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  if (
    header.getUint32(36, true) !== jobResults.length ||
...
function projectJob(
  ...
  const vertices: ResultVertex[] = selected.vertices.map((index) => {
  ...
  const directedFragments: DirectedFragment[] = selected.fragments.map((index) => {
  ...
  const rings: ResultRing[] = selected.rings.map((index) => {
    const value = required(records.rings[index], "result ring");
    const fragmentIds = sliceRange(
      records.fragmentReferences,
      value.referenceBegin,
      value.referenceCount,
      "ring fragments",
    ).map((fragment) => required(records.fragments[fragment], "ring fragment").id);
    return {
      ring_id: value.id,
  ...
  const resultRegions: ResultRegion[] = selected.regions.map((index) => {
  ...
  const operandOutcomes = selected.events.map((index) =>
    projectEvent(records, required(records.events[index], "operand event")),
  );
  return {
    job_id: job.jobId,
    status: "success",
...
function projectDiagnostic(value: DiagnosticRecord): JobDiagnostic {
...
function projectSourceSet(records: ResultRecords, handle: number): SourceSet {
  if (handle === 0) return { sources: [] };
  const set = required(records.sourceSets[handle - 1], "source-set handle");
  return {
    sources: sliceRange(records.sourceIndices, set.begin, set.count, "source-set indices").map(
      (index) => {
...
function projectEvent(records: ResultRecords, value: EventRecord): OperandOutcomeEvent {
  const kind = outcomeKinds[value.kind];
  if (kind === undefined) fail("Unknown operand outcome kind.");
  const ringIds: bigint[] = [];
  ...
  for (const reference of sliceRange(
    ...
    const index = Number(reference & 0xffff_ffffn);
    if (referenceKind === 1) ringIds.push(required(records.rings[index], "outcome ring").id);
    else if (referenceKind === 2)
      regionIds.push(required(records.regions[index], "outcome region").id);
    else fail("Unknown ring/region reference kind.");
  }
  return {
    operand_id: value.operandId,
    kind,
...
function projectRelationship(
  records: ResultRecords,
  value: RelationshipRecord,
): PlanarRelationshipResult {
  if (value.status > 1 || value.dimension > 3) fail("Unknown relationship status or dimension.");
  const dimensions = ["disjoint", "point", "curve", "area"] as const;
  return {
    ...
    pairs: sliceRange(records.pairs, value.pairBegin, value.pairCount, "relationship pairs").map(
      (pair) => ({
...
function validateResultRecords(
  records: ResultRecords,
...
function encodeResultRecords(records: ResultRecords): Uint8Array {
...
function decodeDirectory(
  ...
  for (let index = 0; index < recordSizes.length; index += 1) {
    ...
    if (
      view.getUint16(entry, true) !== kind ||
    ...
    for (let at = end; at < cursor; at += 1)
      if (view.getUint8(at) !== 0) fail("Packet alignment padding is nonzero.");
  }
...
function encodeTables(
  ...
  if (cursor > MAX_PACKET_BYTES) fail("Packet exceeds the A0 attachment bound.");
...
function recordView(table: DirectoryTable, index: number): DataView {
  if (!Number.isSafeInteger(index) || index < 0 || index >= table.count)
    fail(`Invalid index into result table ${table.kind}.`);
  return new DataView(
    table.view.buffer,
    table.view.byteOffset + table.offset + index * table.recordBytes,
    table.recordBytes,
  );
}
function mapTable<T>(
  table: DirectoryTable | undefined,
  project: (view: DataView, index: number) => T,
): T[] {
  const value = required(table, "result table");
  return Array.from({ length: value.count }, (_, index) =>
    project(recordView(value, index), index),
  );
}
...
function putU32(view: DataView, offset: number, value: number, label: string): void {
...
function putI64(view: DataView, offset: number, value: bigint, label: string): void {
  if (typeof value !== "bigint" || value < I64_MIN || value > I64_MAX)
    fail(`${label} must be an int64 bigint.`);
  view.setBigInt64(offset, value, true);
}
function unsigned(value: bigint, label: string, nonzero: boolean): void {
  if (typeof value !== "bigint" || value < (nonzero ? 1n : 0n) || value > U64_MAX)
    fail(`${label} must be a${nonzero ? " nonzero" : ""} uint64 bigint.`);
}
function length(value: bigint, label: string): void {
  unsigned(value, label, true);
  if (value > MAX_LENGTH_NM) fail(`${label} exceeds ${MAX_LENGTH_NM} nanometers.`);
}
function compareBigint(left: bigint, right: bigint): number {
  return left < right ? -1 : left > right ? 1 : 0;
}
function compareSource(left: SourceRecord, right: SourceRecord): number {
...
function compareNumberArrays(left: readonly number[], right: readonly number[]): number {
  ...
  for (let index = 0; index < common; index += 1) {
    const comparison =
      required(left[index], "left member") - required(right[index], "right member");
    if (comparison !== 0) return comparison;
  }
  return left.length - right.length;
}
...
function strictlyIncreasing(values: readonly bigint[], label: string): void {
  for (let index = 1; index < values.length; index += 1)
    if (required(values[index - 1], label) >= required(values[index], label))
      fail(`${label} are not strictly increasing.`);
}
function oneBased(values: readonly bigint[], label: string): void {
  values.forEach((value, index) => {
    if (value !== BigInt(index + 1)) fail(`${label} ids are not canonical one-based ordinals.`);
  });
}
function partition(
  ...
  for (const [begin, count] of ranges) {
    if (count === 0) {
      if (begin !== 0) fail(`Empty ${label} range must begin at zero.`);
    } else {
      if (begin !== cursor || begin + count > total) fail(`${label} ranges are not gapless.`);
      cursor += count;
    }
  }
  if (cursor !== total) fail(`${label} ranges do not own their complete table.`);
}
...
function flags(size: number): boolean[] {
  return Array.from({ length: size }, () => false);
}
function sparseMap(indexes: readonly number[]): Map<number, number> {
...
function mapped(mapping: ReadonlyMap<number, number>, index: number): number {
  const value = mapping.get(index);
  if (value === undefined) fail("Standalone closure contains an unmapped reference.");
  return value;
}
function remapHandle(handle: number, mapping: ReadonlyMap<number, number>): number {
