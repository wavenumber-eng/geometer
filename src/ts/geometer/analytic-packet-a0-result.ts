...
} from "./analytic-packet-a0-validation.js";
import type {
...
function preflightLogicalSourceReferenceExpansions(tables: readonly DirectoryTable[]): void {
  ...
  let total = 0;
  const charge = (handle: number): void => {
    if (handle === 0) return;
    if (handle > sourceSets.count) fail("Logical source-set handle is out of range.");
