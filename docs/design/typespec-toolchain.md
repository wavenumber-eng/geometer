...
the governed catalog, schemas, and HTML reference.
...
replays governed pilot vectors. Output uses UTF-8, LF endings, stable key/order
rules, and one trailing newline. Absolute paths, timestamps, host details, and
sibling-workspace paths are forbidden in generated bytes.

The TypeSpec/catalog foundation and generated contract documentation have
dedicated Rack strata. Later language and transport strata consume the same
committed catalog and schemas; they do not recompile a private subset or
maintain handwritten root/operation lists.
