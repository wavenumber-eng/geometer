...
shared stylesheet have no local changes. This is the migration roadmap and HTML-generation design record. Runtime
contract migrations remain follow-on implementation, not documentation edits.
...
Extend this generator instead of replacing it. The documentation cleanup has
corrected these two originally observed gaps:

- Operation HTML previously showed `native_runtime_available` directly as native executable
...
- The generator previously read lifecycle `status` without displaying the analytic
...
TypeScript renderer and the native C++/IPC implementation in 2026.9.6. Colored
...
The [public entry-point reconciliation](public-entrypoints.md) maps current
user-facing families, aliases, native helper boundaries and client exports.
The source-review lock detects new/changed public sources requiring reassessment.
Migration remains follow-on work even where documentation reconciliation is done.
...
matching Geometer's recorded upstream snapshot. Before this cleanup, the
differences were:
...
This cleanup adopts the ALX teal palette and common layout/component rules
across generated Geometer documentation. Retain the public-distribution Cousine font
...
Record each authority transition in the promotion manifest with its evidence.
Do not mark a migration wave complete based solely on generated documentation.
