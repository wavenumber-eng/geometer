import { compareNumberArrays, compareSource, diagnosticCodes, encodeTables, fail, flags, MAX_JOBS, MAX_LENGTH_NM, MAX_QUERIES, mapped, oneBased, outcomeKinds, partition, putU64, RESULT_MAGIC, RESULT_TABLES, record, recordView, remapHandle, required, sliceRange, sparseMap, strictlyIncreasing, U32_NONE, } from "./analytic-packet-a0-common.js";
export function validateResultRecords(records, tables) {
    ...
    const usedFragmentReferences = new Set();
    records.rings.forEach((value, index) => {
        ...
        for (const reference of sliceRange(records.fragmentReferences, value.referenceBegin, value.referenceCount, "ring fragments")) {
...
export function encodeStandalone(input, jobIndex, selection) {
...
export function encodeResultRecords(records) {
